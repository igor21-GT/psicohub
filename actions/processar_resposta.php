...
$email_aluno = trim($_POST['email_aluno']);         // NOVO
$matricula_aluno = trim($_POST['matricula_aluno']); // NOVO
$respostas_aluno = $_POST['respostas'] ?? []; // Array com [questao_id => opcao_id]

// Validação simples
if (empty($nome_aluno) || empty($email_aluno) || empty($matricula_aluno)) {
    die("Por favor, volte e preencha todos os seus dados (Nome, Email e Matrícula).");
}
...
try {
    ...
    // 4. Salva no Banco (ATUALIZADO com Email e Matricula)
    $stmtSalvar = $pdo->prepare("INSERT INTO respostas_alunos (quiz_id, nome_aluno, email_aluno, matricula_aluno, nota_final) VALUES (:quiz, :nome, :email, :matr, :nota)");
    $stmtSalvar->execute([
        ':quiz' => $quiz_id,
        ':nome' => $nome_aluno,
        ':email' => $email_aluno,
        ':matr' => $matricula_aluno,
        ':nota' => $nota_final
    ]);
