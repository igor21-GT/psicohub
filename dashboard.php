...
// Gráfico de Turnos
try {
    $stmt = $pdo->query("SELECT turno, COUNT(*) as qtd FROM turmas GROUP BY turno");
    $dados_turno = $stmt->fetchAll(PDO::FETCH_KEY_PAIR);
    
    $qtd_manha = $dados_turno['Manhã'] ?? 0;
    $qtd_noite = $dados_turno['Noite'] ?? 0;
    $total_turnos = $qtd_manha + $qtd_noite;
    
    $perc_manha = $total_turnos > 0 ? round(($qtd_manha / $total_turnos) * 100) : 0;
    $perc_noite = $total_turnos > 0 ? round(($qtd_noite / $total_turnos) * 100) : 0;
    
} catch (Exception $e) { $qtd_manha = 0; $qtd_noite = 0; $perc_manha = 0; $perc_noite = 0; }

// DADOS SIMULADOS DOS PERÍODOS (Faculdade)
// Ajuste isso futuramente para buscar do seu banco: "SELECT periodo, COUNT(*) FROM turmas GROUP BY periodo"
$periodos_ativos = [
    '1º Período' => ['qtd' => 2, 'cor' => 'bg-blue-500'],
    '3º Período' => ['qtd' => 1, 'cor' => 'bg-purple-500'],
    '5º Período' => ['qtd' => 3, 'cor' => 'bg-emerald-500'],
    '7º Período' => ['qtd' => 1, 'cor' => 'bg-rose-500'],
];
...
try {
    // Verifica se a tabela existe antes de tentar buscar (para não quebrar se vc não criou ainda)
...
<style>
    ...
    .turno-bar-container {
        display: flex; height: 25px; width: 100%;
        background-color: #e2e8f0; border-radius: 12px; overflow: hidden;
    }
    body.dark-mode .turno-bar-container { background-color: #334155; }
    
    .turno-segment {
        display: flex; align-items: center; justify-content: center;
        font-size: 0.75rem; color: white; font-weight: 600; transition: width 0.5s ease;
    }
    
    /* Classes utilitárias de cor para os badges dos períodos */
...
<div class="grid-container" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 20px; margin-bottom: 20px;">
    <div class="stat-card" style="background: var(--card-bg); padding: 20px; border-radius: 10px; box-shadow: 0 2px 5px rgba(0,0,0,0.05); display: flex; align-items: center; gap: 15px; border-left: 4px solid #8b5cf6;">
        <div class="stat-icon" style="background: #f5f3ff; color: #8b5cf6; width: 50px; height: 50px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 1.2rem;"><i class="fa-solid fa-chalkboard-user"></i></div>
        <div class="stat-info">
    ...
    <div class="stat-card" style="background: var(--card-bg); padding: 20px; border-radius: 10px; box-shadow: 0 2px 5px rgba(0,0,0,0.05); display: flex; align-items: center; gap: 15px; border-left: 4px solid #10b981;">
        <div class="stat-icon" style="background: #ecfdf5; color: #10b981; width: 50px; height: 50px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 1.2rem;"><i class="fa-solid fa-calendar-check"></i></div>
        <div class="stat-info">
    ...
    <div class="stat-card" style="background: var(--card-bg); padding: 20px; border-radius: 10px; box-shadow: 0 2px 5px rgba(0,0,0,0.05); display: flex; align-items: center; gap: 15px; border-left: 4px solid #f59e0b;">
        <div class="stat-icon" style="background: #fffbeb; color: #f59e0b; width: 50px; height: 50px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 1.2rem;"><i class="fa-solid fa-server"></i></div>
        <div class="stat-info">
            <h3 style="margin: 0; font-size: 1.2rem;">Online</h3>
            <p style="margin: 0; color: var(--text-muted); font-size: 0.9rem;">Status Sistema</p>
        </div>
    </div>
</div>
    ...
    <div style="display: flex; flex-direction: column; gap: 20px;">
        ...
        <div class="card" style="background: var(--card-bg); padding: 20px; border-radius: 10px; box-shadow: 0 2px 5px rgba(0,0,0,0.05);">
            <div style="display: flex; justify-content: space-between; align-items: center;">
                <h3 style="margin: 0;">Visão Geral Acadêmica</h3>
                <span style="font-size: 0.8rem; color: var(--text-muted);"><?php echo $total_turmas; ?> Turmas</span>
            </div>

            <div style="margin-top: 20px; margin-bottom: 20px;">
                <p style="font-size: 0.75rem; color: var(--text-muted); text-transform: uppercase; margin-bottom: 5px;">Por Turno</p>
                <?php if($total_turmas > 0): ?>
                    <div class="turno-bar-container">
                        <div class="turno-segment" style="width: <?php echo $perc_manha; ?>%; background-color: #f59e0b;" title="Manhã: <?php echo $qtd_manha; ?>">
                            <?php if($perc_manha > 15) echo 'Manhã '.$perc_manha.'%'; ?>
                        </div>
                        <div class="turno-segment" style="width: <?php echo $perc_noite; ?>%; background-color: #6366f1;" title="Noite: <?php echo $qtd_noite; ?>">
                            <?php if($perc_noite > 15) echo 'Noite '.$perc_noite.'%'; ?>
                        </div>
                    </div>
                <?php else: ?>
                    <p style="text-align:center; color: var(--text-muted);">Sem dados.</p>
                <?php endif; ?>
            </div>

            <div style="border-top: 1px solid var(--border-color); padding-top: 15px;">
                <p style="font-size: 0.75rem; color: var(--text-muted); text-transform: uppercase; margin-bottom: 10px;">Por Período (Semestre)</p>
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
                    <?php foreach($periodos_ativos as $nome => $dados): ?>
                    <div style="display: flex; justify-content: space-between; align-items: center; background: var(--bg-color); padding: 8px 12px; border-radius: 6px;">
                        <div style="display: flex; align-items: center; gap: 8px;">
                            <div class="<?php echo $dados['cor']; ?>" style="width: 8px; height: 8px; border-radius: 50%;"></div>
                            <span style="font-size: 0.85rem; font-weight: 500; color: var(--text-color);"><?php echo $nome; ?></span>
                        </div>
                        <span style="font-size: 0.8rem; font-weight: bold; color: var(--sidebar-active); background: rgba(99, 102, 241, 0.1); padding: 2px 8px; border-radius: 4px;">
                            <?php echo $dados['qtd']; ?>
                        </span>
                    </div>
                    <?php endforeach; ?>
        ...
        <div class="card" style="background: var(--card-bg); padding: 20px; border-radius: 10px; box-shadow: 0 2px 5px rgba(0,0,0,0.05);">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                ...
                <a href="novo_plano.php" style="background: var(--sidebar-active); color: white; border: none; padding: 6px 12px; border-radius: 5px; cursor: pointer; text-decoration: none; font-size: 0.85rem;">+ Novo</a>
            </div>

            <div style="background: linear-gradient(to right, #fdfbf7, #fff7ed); border-left: 4px solid #f59e0b; padding: 15px; border-radius: 4px; margin-bottom: 20px; color: #78350f; font-size: 0.95rem;">
                <?php echo $dica_hoje; ?>
            </div>

            <h4 style="font-size: 0.8rem; color: var(--text-muted); margin-bottom: 10px; text-transform: uppercase;">Recentes</h4>
            <?php if(count($ultimos_planos) > 0): ?>
                <div style="display: flex; flex-direction: column; gap: 10px;">
                    <?php foreach($ultimos_planos as $plano): ?>
                    <div style="display: flex; align-items: center; justify-content: space-between; padding: 10px; border: 1px solid var(--border-color); border-radius: 8px;">
                        <div>
                            <div style="font-weight: 600; color: var(--text-color); font-size: 0.9rem;"><?php echo htmlspecialchars($plano['titulo']); ?></div>
                            <div style="font-size: 0.75rem; color: var(--text-muted);"><?php echo date('d/m/Y', strtotime($plano['data_criacao'])); ?></div>
                        </div>
                        <a href="#" style="color: var(--sidebar-active);"><i class="fa-solid fa-pen"></i></a>
                    </div>
                    <?php endforeach; ?>
                </div>
            <?php else: ?>
                <p style="color: var(--text-muted); font-size: 0.9rem; font-style: italic;">Nenhum plano ainda.</p>
            <?php endif; ?>
        </div>
